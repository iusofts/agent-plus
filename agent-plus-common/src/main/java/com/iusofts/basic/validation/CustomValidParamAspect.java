...
import com.iusofts.basic.exception.InvalidParamException;
...
public class CustomValidParamAspect {
    ...
    /**
     * 切点：拦截参数带@CustomValidParam的方法（重点处理对象参数）
     */
    @Before("execution(* *(.., @com.iusofts.basic.validation.YzValidated (*), ..))")
    public void validateObjectParam(JoinPoint joinPoint) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            YzValidated annotation = param.getAnnotation(YzValidated.class);
            if (annotation != null) {
