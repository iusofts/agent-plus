...
import com.iusofts.agentplus.basic.excel.ExcelUtil;
import com.iusofts.agentplus.basic.utils.PinyinTool;
import com.iusofts.agentplus.basic.utils.PinyinTool.Type;
...
import com.iusofts.agentplus.system.vo.ShopUserExcelImportVo;
...
public class SysUserExpandServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserExpandService {
    ...
    public int add(SysUserDto user) {
        ...
        } else if (com.iusofts.agentplus.basic.utils.StringUtils.isNotEmpty(user.getPhone()) && !sysUserService.checkPhoneUnique(user)) {
            throw new SystemBusinessException("新增用户'" + user.getUsername() + "'失败，手机号码已存在");
        } else if (com.iusofts.agentplus.basic.utils.StringUtils.isNotEmpty(user.getEmail()) && !sysUserService.checkEmailUnique(user)) {
